...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const { grid, containerHeight } = useMemo<{ grid: GridItem[]; containerHeight: number }>(() => {
    ...
    const maxHeight = Math.max(...colHeights) - gap; // Subtract last gap
  ...
  return (
    <div 
      ...
      style={{ height: containerHeight > 0 ? `${containerHeight}px` : 'auto', minHeight: containerHeight > 0 ? `${containerHeight}px` : '400px' }}
    >
      {grid.map((item) => (
        <div
          key={item.id}
